...
schema.getQueryPaging = async function (args, isNFilter = true) {
    let page = args.page || {}
...
schema.getQueryParams = async function (
    args,
    words = ['name'],
    isNFilter = true
) {
    ...
    if (isNFilter && args.n_filter) {
        if (args.n_filter['owner_id'])
            filter['owner_id'] = schema.objectId(args.n_filter['owner_id'])
