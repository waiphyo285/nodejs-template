...
const getMenuPath = (locale) => {
    const menuConfig = {
        en_US: `../../resources/lang/menus/menu-en.json`,
        my_MM: `../../resources/lang/menus/menu-my.json`,
    }

    return menuConfig[locale] || menuConfig['en_US']
}
...
const getProgram = async (user, pageId) => {
    ...
    if (userRole === 'developer') {
        const developerProgram = initProgram.map((initMenu) => {
            ...
            initMenu.submenu = initMenu.submenu.map((initSubMenu) => {
                ...
                if (initSubMenu?.menuid === 'public_user') {
    ...
    // other account based on role & level
        ...
        const result = await roleService.findById(user.level_id)
...
const getPageData = (getProgramMenu, getPageId) => {
    if (getPageId !== undefined) {
        ...
        if (!pageMenuObj || !pageMenuObj.submenu) {
            return {}
        }
        ...
        return pageSubMenu ? { ...pageSubMenu, active: true } : {}
    }
    return {}
}
